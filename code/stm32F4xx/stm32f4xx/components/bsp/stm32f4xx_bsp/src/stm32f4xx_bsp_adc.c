...
static uint16_t bsp_adc_register_buf[BSP_ADC_COUNT][BSP_ADC_REGISTER_BUFSIZE] = { 0 };
/**
 * @}
 */
...
void BSP_ADC_Start(uint8_t BSP_ADCx)
{
	switch(BSP_ADCx)
	{
		case BSP_ADC1:ADC_Cmd( ADC1, ENABLE);break;
		default:break;
	}
}
void BSP_ADC_Stop(uint8_t BSP_ADCx)
{
	switch(BSP_ADCx)
	{
		case BSP_ADC1:ADC_Cmd( ADC1, DISABLE);break;
		default:break;
	}
}
...
void BSP_ADC1_DMA_IRQHandler(void)
{
	if(DMA_GetITStatus(DMA2_Stream0, DMA_IT_TCIF0) == SET)
	{
		...
		//BSP_ADC_Stop(BSP_ADC1);
		//BSP_ADC_Start(BSP_ADC1);
		//ADC_SoftwareStartConv(ADC1);
	}
}
